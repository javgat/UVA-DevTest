...
export class LqFavsUserComponent extends ListQuestionsComponent implements OnInit {
  ...
  getQuestionsInclude(primera: boolean) {
    ...
    this.userS.getFavoriteQuestions(this.id, this.searchTags, this.likeTitle).subscribe(
  ...
  getQuestionsEdit(primera: boolean) {
    if(this.id==undefined) return
    this.userS.getFavoriteEditQuestions(this.id, this.searchTags, this.likeTitle).subscribe(
      resp => this.questions = resp,
