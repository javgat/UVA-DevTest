...
@Component({
  selector: 'app-lt-profile-public',
  templateUrl: '../list-tests.component.html',
  styleUrls: ['../list-tests.component.css']
})
export class LtProfilePublicComponent extends ListTestsComponent implements OnInit {
  ...
  getTestsInclude(primera: boolean) {
    //No hay getPublicTestsFromUser en la API que tenga filtros
    this.getTestsEdit(true)
  }

  getTestsEdit(primera: boolean) {
    if(this.id==undefined) return
    this.userS.getPublicEditTestsFromUser(this.id, this.searchTags, this.likeTitle).subscribe(
